...
/*
 * Direct integer-N PLL programming for source-defined operating points.
 * The caller owns the PMS values; no lookup or nearest-rate synthesis occurs.
 */
int ra_set_pll_pms(unsigned int id, unsigned int m,
                   unsigned int p, unsigned int s)
{
    struct cmucal_clk *clk;
    struct cmucal_pll *pll;
    struct cmucal_pll_table table = { 0, };
    struct cmucal_clk *umux;
    int ret;
    ...
    if (!p || m > width_to_mask(pll->m_width) ||
        p > width_to_mask(pll->p_width) ||
        s > width_to_mask(pll->s_width))
        return -EVCLKINVAL;
    ...
    /* Same safe mux/disable/program/restore sequence as ra_set_pll(). */
    ra_enable_pll(clk, 0);
    ret = ra_pll_set_pmsk(clk, &table);
    if (ret)
        return ret;
...
EXPORT_SYMBOL_GPL(ra_set_pll_pms);

int ra_get_pll_pms(unsigned int id, unsigned int *m,
                   unsigned int *p, unsigned int *s)
{
    struct cmucal_clk *clk;
    struct cmucal_pll *pll;
    ...
    pll = to_clk_pll(clk);
    con0 = readl(clk->pll_con0);

    *m = (con0 >> pll->m_shift) & width_to_mask(pll->m_width);
    *p = (con0 >> pll->p_shift) & width_to_mask(pll->p_width);
    *s = (con0 >> pll->s_shift) & width_to_mask(pll->s_width);

    return 0;
}
EXPORT_SYMBOL_GPL(ra_get_pll_pms);
